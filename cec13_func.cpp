...
void mexFunction (int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[]) 
{
	...
	m = mxGetN (prhs[0]); //å¾??°ç?©é?µç??è¡?
	x = mxGetPr (prhs[0]); //?·å???©é?µç??ä¸?ä¸???ç´???????
	...
	plhs[0] = mxCreateDoubleMatrix (1, m, mxREAL); //ç¬?ä¸?ä¸?è¾??ºæ??1*m???©é?µï?è¿???????ä¸?ä¸?è¾??ºé¡¹ï¼?
	f = mxGetPr (plhs[0]); //?·å???©é?µç??ä¸?ä¸???ç´???????
	test_func(&x[0], &f[0], n,m,func_num); //è¾??¥ç?©é?µï?è¾??ºç?©é?µï?è¾??¥ç?©é?µè?ï¼???ï¼?æµ?è¯??¨ä?ç¼???
...
void test_func(double *x, double *f, int nx, int mx,int func_num)
{
	...
	{
		FILE *fpt;
		char FileName[30]; //char FileName[PATH_MAX];
		free(M);
		free(OShift);
		free(y);
		free(z);
		...
		{
				ret = fscanf(fpt,"%lf",&M[i]); //å°??°æ??è¯»å??M?©é??
				if (ret != 1)
				{
		...
		sprintf(FileName, "input_data/shift_data.txt");
		fpt=fopen(FileName,"r");
		if (fpt==NULL)
		{
			printf("\nError: Cannot open input file for reading\n");
		...
		{
				ret = fscanf(fpt,"%lf",&OShift[i]); //å°???10*nx???°æ??è¯»å??Oshift
				if (ret != 1)
				{
				    printf("\nError: Error reading from the input file\n");
				}
